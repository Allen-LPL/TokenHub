...
请求复用项目鉴权、模型权限、配额、路由选择、Provider 凭据、错误分类、用量保存、请求日志和 Trace 导出。隐私和安全钩子收到 `route_protocol: "systemone"`。确定性出站检查覆盖状态、问题 ID、指令和判断标准，包括嵌套对象键和 JSON 数字叶子值。数字保留原始精度；值为整数的科学计数法还会以最多 64 个字符的展开形式接受检查。若策略要求脱敏结构性键或数字值，网关会拒绝请求，因为文本掩码无法保持答案契约或 JSON 类型。可选的请求前处理、路由、Provider 调用、响应和用量钩子遵循已有网关生命周期。本阶段不对 System One 响应启用缓存。

读取上游响应体时发生的调用方取消会保留为取消错误，不触发故障切换。`422` 等客户端错误不触发故障切换；上游 `429`、`529` 等暂时性错误可按既有策略切换到其他兼容路由。答案缺失或格式错误、概率或评分越界、用量缺失或无效均返回脱敏后的 `502`。失败尝试已报告的有效用量会保留。上游凭据错误属于 Provider 错误，不会伪装成项目 Key 鉴权失败。
