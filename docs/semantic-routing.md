...
Candidate catalog entries must be active. Declared modalities, supported wire parameters and context limits constrain eligibility; a parameter from another protocol is not treated as equivalent. Context checking uses a conservative serialized-byte estimate plus the output-token budget. Missing declarations do not establish capability or suitability. Keep Provider model metadata accurate.
